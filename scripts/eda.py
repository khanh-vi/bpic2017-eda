...
def build_prediction_prefix(dataframe, k=K):
    ...
    df_work["event_nr"] = df_work.groupby(
        CASE_COL,
        sort=False,
    ).cumcount() + 1
    ...
    outcome_inside_prefix = prefix_k10[ACTIVITY_COL].isin(
        OUTCOME_ACTIVITIES
    ).sum()
    ...
    summary = {
        ...
        "outcome_inside_prefix": int(outcome_inside_prefix),
    }
    ...
    print(prefix_sizes.value_counts().sort_index())
